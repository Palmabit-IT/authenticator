...
class UserRegisterService
{
	...
	protected function sendMailToAdmins($mailer, $user, $input)
	{
		...
		if (!empty($mails)) {
			foreach ($mails as $mail) {
				$mailer->sendTo($mail, array_merge(["email" => $input["email"], "id" => $user->id],
					$this->getArrayFieldToSend($input)), "User signup request", "authentication::mail.registration-waiting-admin");
			}
		}
	}
	...
	public function getArrayFieldToSend($input)
	{
		$fields = Config::get('authentication::field_to_send_for_email_new_registration_admin');
		$result = [];
		foreach ($fields as $field) {
			if (array_key_exists($field, $input)) {
				$result = array_merge($result,[$field => $input[$field]]);
			}
		}
