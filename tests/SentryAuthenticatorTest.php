...
class SentryAuthenticatorTest extends DbTestCase
{
  ...
  public function it_check_for_user_groups()
  {
    ...
    $mock_group  = m::mock('StdClass')->shouldReceive('inGroup')->andReturn(true)->getMock();
    $mock_sentry = m::mock('StdClass')->shouldReceive('getUser')->once()->andReturn($mock_group)->getMock();
    App::instance('sentry', $mock_sentry);
    $mock_repo = m::mock('StdClass')->shouldReceive('findByName')->once()->andReturn($group)->getMock();
  ...
  public function it_gets_logged_user_profile()
  {
    ...
    $get_mock          = m::mock('StdClass')->shouldReceive('first')->once()->andReturn($expected_profile)->getMock();
    $mock_user_profile = m::mock('StdClass')->shouldReceive('user_profile')->once()->andReturn($get_mock)->getMock();

    $mock_sentry = m::mock('StdClass')->shouldReceive('getUser')->once()->andReturn($mock_user_profile)->getMock();
    App::instance('sentry', $mock_sentry);
